...
pub fn create<const IS_CREATE2: bool, SPEC: Spec>(
    ...
    use crate::StuckReason::{Execute, Create, CreateReturn};
    if matches!(interpreter.stuck_reason, Execute) {
        ...
        interpreter.stuck_reason = Create(create_input);
    } else {
        let CreateReturn(return_reason, address, gas, return_data) = 
            std::mem::replace(&mut interpreter.stuck_reason, Execute) else {
            panic!("why create is called when the state is neither execute nor create return?")
        };
...
pub fn call_inner<SPEC: Spec>(
    ...
    use crate::StuckReason::{Call, CallReturn, Execute};
    if matches!(interpreter.stuck_reason, Execute) {
        match scheme {
